...
Use the generic `OverflowDetector` to detect when a menu item label is truncated and react accordingly (e.g. show a tooltip).
The `OverflowDetector` is not specific to menu items and can be used on any element.

```js
import { OverflowDetector } from "@user/src/js/overflow-detector"

const detector = new OverflowDetector()

document.querySelectorAll(".blue-menu-item .text-truncate").forEach((label) => {
    detector.observe({
        element: label,
        onChange: (isOverflowing, el) => {
            const menuItem = el.closest(".blue-menu-item")
            if (!menuItem) return

            if (isOverflowing) {
                menuItem.setAttribute("data-tooltip", el.textContent?.trim() ?? "")
...
<div style="max-width: 14rem">
    <button class="btn blue-menu-item icon-link d-flex w-100">
        ...
        <span class="flex-grow-1 text-truncate" style="min-width: 0">
            This is a very long menu item label that should truncate and show a tooltip
        </span>
    </button>
</div>
```
